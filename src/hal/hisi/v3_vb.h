...
static int v3_vb_load(v3_vb_impl *vb_lib) {
    if (!(vb_lib->handle = dlopen("libmpi.so", RTLD_LAZY | RTLD_GLOBAL))) {
        fprintf(stderr, "[v3_vb] Failed to load library!\nError: %s\n", dlerror());
        return EXIT_FAILURE;
    }

    if (!(vb_lib->fnConfigPool = (int(*)(v3_vb_pool *config))
        dlsym(vb_lib->handle, "HI_MPI_VB_SetConf"))) {
        fprintf(stderr, "[v3_vb] Failed to acquire symbol HI_MPI_VB_SetConf!\n");
        return EXIT_FAILURE;
    }

    if (!(vb_lib->fnConfigSupplement = (int(*)(v3_vb_supl *value))
        dlsym(vb_lib->handle, "HI_MPI_VB_SetSupplementConf"))) {
        fprintf(stderr, "[v3_vb] Failed to acquire symbol HI_MPI_VB_SetSupplementConf!\n");
        return EXIT_FAILURE;
    }

    if (!(vb_lib->fnExit = (int(*)(void))
        dlsym(vb_lib->handle, "HI_MPI_VB_Exit"))) {
        fprintf(stderr, "[v3_vb] Failed to acquire symbol HI_MPI_VB_Exit!\n");
        return EXIT_FAILURE;
    }

    if (!(vb_lib->fnInit = (int(*)(void))
        dlsym(vb_lib->handle, "HI_MPI_VB_Init"))) {
        fprintf(stderr, "[v3_vb] Failed to acquire symbol HI_MPI_VB_Init!\n");
        return EXIT_FAILURE;
    }
