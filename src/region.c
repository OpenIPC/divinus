...
void *region_thread(void)
{
    ...
    while (keepRunning) {
        for (char id = 0; id < MAX_OSD; id++) {
            if (!empty(osds[id].text) && !osds[id].updt)
            {
                char out[80];
                    ...
                    if (!access(font, F_OK)) {
                        ...
                        switch (plat) {
                            ...
                            case HAL_PLATFORM_V3:
                                v3_region_create(osds[id].hand, rect);
                                v3_region_setbitmap(osds[id].hand, &bitmap);
                                break;
                        }
                        free(bitmap.data);
                    }
