...
extern v2_config_impl v2_config;

static enum ConfigError v2_parse_config_lvds(
    struct IniConfig *ini, const char *section, v2_snr_lvds *lvds) {
...
static enum ConfigError v2_parse_config_videv(
    struct IniConfig *ini, const char *section, v2_vi_dev *device) {
    enum ConfigError err;
    ...
    }
    {
        ...
        err = parse_enum(
            ...
            count, 0);
        if (err != CONFIG_OK)
            device->work = V2_VI_WORK_1MULTIPLEX;
    }
...
static enum ConfigError v2_parse_config_vichn(
    struct IniConfig *ini, const char *section, v2_vi_chn *channel) {
    enum ConfigError err;
...
static enum ConfigError v2_parse_config_isp(
    struct IniConfig *ini, const char *section, v2_isp_dev *isp) {
    enum ConfigError err;
...
static enum ConfigError v2_parse_sensor_config(char *path, v2_config_impl *config) {
    if (config == NULL)
    ...
    // [mode]
    {
        ...
        err = parse_enum(
            ...
            possible_values, count, 0);
        if (err != CONFIG_OK)
            config->input_mode = V2_SNR_INPUT_MIPI;
    }

    if (config->input_mode == V2_SNR_INPUT_MIPI) {
        // [mipi]
        {
    ...
    } else if (config->input_mode == V2_SNR_INPUT_LVDS) {
        // [lvds]
        err = v2_parse_config_lvds(&ini, "lvds", &config->lvds);
        if (err != CONFIG_OK)
            goto RET_ERR;
    }

    // [isp_image]
    err = v2_parse_config_isp(&ini, "isp_image", &config->isp);
    if (err != CONFIG_OK)
        goto RET_ERR;

    // [vi_dev]
    err = v2_parse_config_videv(&ini, "vi_dev", &config->videv);
    if (err != CONFIG_OK)
        goto RET_ERR;
    // [vi_chn]
    err = v2_parse_config_vichn(&ini, "vi_chn", &config->vichn);
    if (err != CONFIG_OK)
        goto RET_ERR;
