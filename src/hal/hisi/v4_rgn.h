...
static int v4_rgn_load(v4_rgn_impl *rgn_lib) {
    ...
    if (!(rgn_lib->fnAttachChannel = (int(*)(unsigned int handle, v4_sys_bind *dest, v4_rgn_chn *config))
        ...
        fprintf(stderr, "[v4_rgn] Failed to acquire symbol HI_MPI_RGN_AttachToChn!\n");
        return EXIT_FAILURE;
    }

    if (!(rgn_lib->fnDetachChannel = (int(*)(unsigned int handle, v4_sys_bind *dest))
        dlsym(rgn_lib->handle, "HI_MPI_RGN_DetachFromChn"))) {
        fprintf(stderr, "[v4_rgn] Failed to acquire symbol HI_MPI_RGN_DetachFromChn!\n");
        return EXIT_FAILURE;
    }
    ...
    if (!(rgn_lib->fnGetChannelConfig = (int(*)(unsigned int handle, v4_sys_bind *dest, v4_rgn_chn *config))
        ...
        fprintf(stderr, "[v4_rgn] Failed to acquire symbol HI_MPI_RGN_GetDisplayAttr!\n");
        return EXIT_FAILURE;
    }
    ...
    if (!(rgn_lib->fnSetChannelConfig = (int(*)(unsigned int handle, v4_sys_bind *dest, v4_rgn_chn *config))
        ...
        fprintf(stderr, "[v4_rgn] Failed to acquire symbol HI_MPI_RGN_SetDisplayAttr!\n");
        return EXIT_FAILURE;
    }

    if (!(rgn_lib->fnSetBitmap = (int(*)(unsigned int handle, v4_rgn_bmp *bitmap))
        dlsym(rgn_lib->handle, "HI_MPI_RGN_SetBitMap"))) {
        fprintf(stderr, "[v4_rgn] Failed to acquire symbol HI_MPI_RGN_SetBitMap!\n");
        return EXIT_FAILURE;
    }
