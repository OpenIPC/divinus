...
typedef struct {
    ...
    int (*fnGetPadInfo)(int sensor, i6c_snr_pad *info);
    int (*fnGetPlaneInfo)(int sensor, unsigned int index, i6c_snr_plane *info);

    int (*fnCurrentResolution)(int sensor, unsigned char *index, i6c_snr_res *resolution);
    int (*fnGetResolution)(int sensor, unsigned char index, i6c_snr_res *resolution);
    int (*fnGetResolutionCount)(int sensor, unsigned int *count);
    int (*fnSetResolution)(int sensor, unsigned char index);
} i6c_snr_impl;

int i6c_snr_load(i6c_snr_impl *snr_lib) {
    ...
    if (!(snr_lib->fnGetPadInfo = (int(*)(int sensor, i6c_snr_pad *info))
        dlsym(snr_lib->handle, "MI_SNR_GetPadInfo"))) {
        fprintf(stderr, "[i6c_snr] Failed to acquire symbol MI_SNR_GetPadInfo!\n");
        return EXIT_FAILURE;
    }

    if (!(snr_lib->fnGetPlaneInfo = (int(*)(int sensor, unsigned int index, i6c_snr_plane *info))
        dlsym(snr_lib->handle, "MI_SNR_GetPadInfo"))) {
        fprintf(stderr, "[i6c_snr] Failed to acquire symbol MI_SNR_GetPadInfo!\n");
        return EXIT_FAILURE;
    }

    if (!(snr_lib->fnCurrentResolution = (int(*)(int sensor, unsigned char *index, i6c_snr_res *resolution))
        dlsym(snr_lib->handle, "MI_SNR_GetCurRes"))) {
        fprintf(stderr, "[i6c_snr] Failed to acquire symbol MI_SNR_GetCurRes!\n");
        return EXIT_FAILURE;
    }

    if (!(snr_lib->fnGetResolution = (int(*)(int sensor, unsigned char index, i6c_snr_res *resolution))
...
void i6c_snr_unload(i6c_snr_impl *snr_lib) {
    if (snr_lib->handle)
