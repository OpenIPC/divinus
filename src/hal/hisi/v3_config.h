...
static enum ConfigError v3_parse_config_videv(
    ...
    }
    {
        ...
        err = parse_enum(
            ...
            count, 0);
        if (err != CONFIG_OK)
            device->work = V3_VI_WORK_1MULTIPLEX;
    }
...
data_seq_ok:
    ...
    if (device->intf == V3_VI_INTF_MIPI || device->intf == V3_VI_INTF_LVDS)
        device->rgbModeOn = 1;
...
static enum ConfigError v3_parse_sensor_config(char *path, v3_config_impl *config) {
    ...
    // [mode]
    {
        ...
        err = parse_enum(
            ...
            possible_values, count, 0);
        if (err != CONFIG_OK)
            config->input_mode = V3_SNR_INPUT_MIPI;
    }
    ...
    } else if (config->input_mode == V3_SNR_INPUT_LVDS) {
        // [lvds]
    ...
    // [isp_image]
    err = v3_parse_config_isp(&ini, "isp_image", &config->isp);
    if (err != CONFIG_OK)
        goto RET_ERR;
